...
def inp_val():
  ...
  # else:
    ...
    image_files_list = []
    label_files_list = []
    training_data = os.listdir(os.path.join(path_to_data,input_dataset_type.split('.')[0]))
    for data in training_data:
        if data !='labels.txt':
            if imghdr.what(os.path.join(path_to_data,input_dataset_type.split('.')[0],data)) is None:
